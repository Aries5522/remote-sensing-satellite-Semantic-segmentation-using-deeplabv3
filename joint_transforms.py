...
class RandomHorizontallyFlip(object):           ##随机翻转
    def __call__(self, img, mask):
...
class FreeScale(object):                            ##随机大小，双线性插值函数
    def __init__(self, size):
...
class RandomSizedCrop(object):                          ##随机尺度裁剪
    def __init__(self, size):
        self.size = size
...
class Rotate(object):
    ...
    def __call__(self,img,mask):
        if random.random()>0.5:
            degree=self.degree
            return img.rotate(degree,Image.BILINEAR),mask.rotate(degree,Image.NEAREST)
        return img,mask


class RandomSized(object):                                  ##随机大小
    def __init__(self, size):
        self.size = size
...
class SlidingCropOld(object):                                   ##滑窗
...
class SlidingCrop(object):                      ##不知道在干嘛
    def __init__(self, crop_size, stride_rate, ignore_label):
        self.crop_size = crop_size
        self.stride_rate = stride_rate
        self.ignore_label = ignore_label
