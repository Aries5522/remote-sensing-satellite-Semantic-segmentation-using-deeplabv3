...
def evaluate(predictions, gts, num_classes):
    ...
    pe=np.dot(np.sum(hist,axis=0),np.sum(hist,axis=1))/(hist.sum()*hist.sum())
    kappa=(acc-pe)/(1-pe)

    return acc, acc_cls, mean_iu, fwavacc, single_cls_acc,kappa
...
def compute_datast_weight(data_loader,num_class):
    batch_size1 = 20
    train_loader = DataLoader(data_loader, batch_size=batch_size1)
    weight = np.zeros((1, num_class)).squeeze(0).astype(np.uint)
    for index, data in enumerate(train_loader):
        img, label = data
        label = np.array(label).reshape(256 * 256 * batch_size1, 1).squeeze(1).astype(np.uint8)
        weight_ = np.bincount(label)
